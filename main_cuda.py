...
"""
使用本地 CUDA/Transformers 格式 Qwen3-ASR + Qwen3-ForcedAligner，从视频生成断句合理的 SRT 字幕。

依赖：
  pip install -U qwen-asr torch torchaudio
  Windows 请另外安装 ffmpeg，并确保 ffmpeg/ffprobe 在 PATH 中。

示例：
  python main_cuda.py input.mp4 -o output.srt --language Japanese
...
ASR_MODEL_PATH = r"D:\Download\LLM_Modeds\Qwen3-ASR-1.7B"
ALIGN_MODEL_PATH = r"D:\Download\LLM_Modeds\Qwen3-ForcedAligner-0.6B"
...
def require_ffmpeg() -> None:
    if not shutil.which("ffmpeg"):
        raise RuntimeError("找不到 ffmpeg。请先安装 ffmpeg，并把 ffmpeg.exe 所在目录加入 PATH。")

    if not shutil.which("ffprobe"):
        raise RuntimeError("找不到 ffprobe。ffmpeg 安装后通常会自带 ffprobe，请确认它也在 PATH 中。")


def cleanup_after_inference() -> None:
    """
    尽量释放每个 chunk 推理后的临时内存和 CUDA cache。
    """
    gc.collect()
...
def resolve_torch_dtype(dtype_name: str):
    ...
    raise ValueError("--dtype 只支持 bfloat16 / float16 / float32")
...
def require_cuda_device(device: str) -> None:
    ...
    if device.startswith("cuda") and not torch.cuda.is_available():
        raise RuntimeError(
            "当前 PyTorch 没有检测到 CUDA。请确认安装的是 CUDA 版 torch，"
            "并且 NVIDIA 驱动可用。"
        )
...
class CudaASRModel:
    ...
    def generate(self, audio_path: str, language: str):
...
class CudaForcedAlignModel:
    def __init__(
        ...
        dtype: str = "bfloat16",
        attn_implementation: str = "sdpa",
        max_text_tokens_per_batch: int = 600,
    ) -> None:
        require_cuda_device(device)

        from qwen_asr import Qwen3ForcedAligner

        self.max_text_tokens_per_batch = max_text_tokens_per_batch
...
def load_cuda_asr_model(args) -> CudaASRModel:
    print(f"Loading CUDA ASR model: {args.asr_model}")
    return CudaASRModel(
...
def load_cuda_align_model(args) -> CudaForcedAlignModel:
    print(f"Loading CUDA ForcedAligner model: {args.align_model}")
    return CudaForcedAlignModel(
        model_path=args.align_model,
        device=args.device,
        dtype=args.dtype,
        attn_implementation=args.attn_implementation,
        max_text_tokens_per_batch=args.max_text_tokens_per_batch,
    )
...
def main() -> None:
    ...
    parser.add_argument("--device", default="cuda:0", help="PyTorch device，例如 cuda:0")
    parser.add_argument("--dtype", default="bfloat16", choices=["bfloat16", "float16", "float32"])
    parser.add_argument(
        "--attn-implementation",
        default="sdpa",
        help="Transformers attention backend；Windows 默认 sdpa，已安装 flash-attn 可改为 flash_attention_2",
    )
    ...
    if args.parallel_first_two_only:
        ...
        print("Note: CUDA debug mode runs sequentially so you can validate one GPU pipeline before the full job.")
    ...
    if args.workers == 1:
        ...
        print("Note: this roughly doubles model memory usage. If VRAM is tight, use --workers 1.")

        worker_chunks = [chunks[0::2], chunks[1::2]]

        def run_worker(worker_id: int, assigned_chunks: List[Tuple[int, Path, float]]) -> List[int]:
            print(f"Worker {worker_id}: loading CUDA models")
            worker_asr_model = load_cuda_asr_model(args)
            worker_align_model = load_cuda_align_model(args)
